...
var texto = "Bienvenido a la lección 2, ahora aprenderemos nuevas formas de representar el espacio geográfico. Veamos los tipos en que se representa el espacio geográfico."+"    Da click sobre iniciar" ;
var texto2 = $("#user").text() + " presta mucha atención."
var continentalText = "En el planisferio y en los mapas continentales, se representa reducida la extensión territorial de los países. y se muestra un continente completo  como:  "+" América, "+"  Africa, " +" Asia, "+" Oceanía, "+ " Europa ";
...
var estatalText = "Un mapa estatal muestra un estado o regíon perteneciente a un país ";
var municipalText = "Un mapa municipal muestra las regiones en que se divide un estado  ";
// nombre  del personaje
var personajeName= "Paco";
var ayudaOli = $("#user").text()+" ayuda a "+personajeName+" a llegar a su escuela, tienes que indicarle donde está su escuela ";
...
var direcionEscuela = { continente: "Americano",
                         ...
                         direcion: "Instituto Tecnológico de Oaxaca"
    ...
    require(["esri/geometry/ScreenPoint"], function(ScreenPoint){
        ...
        if (punto) {
            if(punto.latitude && punto.longitude){
                var lat = punto.latitude;
                var lng = punto.longitude;  
...
var dinamica = function(){
            ...
            responsive.iniciar();  
                  instrucion="Indícale en el mapa continental en dónde se encuentra el país de " + personajeName;
    ...
    if(banderaContinental ){
       ...
       if(pais.long_name ===  direcionEscuela.pais){
           ...
           intentos= intentos+1;
            ...
            responsive.iniciar();  
             responsive.leer("Te equivocaste "+$("#user").text()+", este país no es "+direcionEscuela.pais+", Vuelve a intentarlo");
           intentos=intentos+1;
...
var googlemap = function(divdemapa){
    ...
    //map.addListener('click', function(event) {
     var clicmap = function(event) {
         console.log("MIRAAAAAAAAA");
         console.log(event);
         require(["esri/geometry/ScreenPoint"], function(ScreenPoint){
        var sp = new ScreenPoint({
            x: evt.x,
        ...
        if (punto) {
         ...
         // se manda  la posicion  a la funcion buscar pais
                geocoder = new google.maps.Geocoder;
                geocoder.geocode({'location': posi }, function(results, status) {
                if (status === google.maps.GeocoderStatus.OK) {
     ...
     document.getElementById("viewDiv").click = clicmap(this);
}
...
var estado = function(results){
  ...
  if(es.long_name === direcionEscuela.estado){
      banderaEstatal= true;
            responsive.iniciar();  
             responsive.leer("Muy bien "+$("#user").text());
            puntuacion=puntuacion+1;
            intentos= intentos+1;
      ...
      // municipal
          ...
          // despues se pide que de cick en la escuela del personaje
                      responsive.iniciar();
             instrucion=$("#user").text()+" indícale en el mapa en dónde se encuentra la escuela de " + personajeName;
             responsive.leer(instrucion);
        ...
        map.setCenter(pos);
            ...
            responsive.iniciar();  
             responsive.leer("Fallaste "+$("#user").text()+", este estado no es "+direcionEscuela.estado+", Vuelve a intentarlo");
           } 
...
{
     responsive.iniciar();  
             responsive.leer("Felicidades "+$("#user").text()+ " has ayudado a "+ personajeName);
     banderaFin= true;
     puntuacion=puntuacion+1;
    intentos= intentos+1;
    ...
    responsive.iniciar();  
             responsive.leer("Incorrecto "+$("#user").text()+ ", vuelve a intentarlo. " );
    intentos= intentos+1;
}
